...
const AirPortCheckedLocation = ({records_L_Location, selectedvalueLocation, selectedvalueStars,
    selectedvalueCarPrand, defaultValue, activeSelected, selectedCarPrand, showListFilter
    }) => {
    // return <></>
    const SizeScreen = useMediaQuery(`(max-width: 1024px)`)
    const params = {
        slidesPerView: `${showListFilter ? SizeScreen ?  2 : 3 : SizeScreen ? 1 :  2}`,
        spaceBetween: 30,
        navigation: {
    ...
    const prand = records_L_Location?.map((item, index) => item.prandtype);
    ...
    useEffect(() => {
        ...
        const filterResult = records_L_Location?.filter((item) => {
    ...
    }, [ records_L_Location, selectedvalueLocation, selectedvalueCarPrand, selectedvalueStars, defaultValue,showListFilter,SizeScreen]);
    if(FilteredResult?.length ){
        return (
        Object.entries(resultObject)?.map(([key, values], index) => {
            return (
                values?.length > 0  &&
                <Swiper {...params} className={`carts  mySwiper `}
                navigation={true} modules={[Navigation]} key={index}
                centeredSlides={values.length <= 1 ? true : false}
                >
                    {
                        values?.map((item, index) => {
                            return (
                                    ...
                                    >
                                        ...
                                        >
                                            <div className={`cart airport_cart`}   data-aos="zoom-in" data-aos-duration="10">
                                                ...
                                                <div className='tardemark'>
                                                    ...
                                                    <div className="trade_img">
                                                        ...
                                                        <div className='cart_stars'>{`Location ${item?.location}`}</div>
                                                    </div>
                                                </div>
                                            ...
                                            <Button className='airport_maincart__rent' variant="contained" disableElevation>Rent a car now</Button>
                                        </div>
                                    </SwiperSlide>
                            );
                        })
        ...
        return (
            <>
                <h1>The Car Brand type is not available in this location</h1>
            </>
        )
    }
}

export default AirPortCheckedLocation